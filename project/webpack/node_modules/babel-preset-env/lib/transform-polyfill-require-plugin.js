  ...
  var isPolyfillImport = {
    ImportDeclaration: function ImportDeclaration(path, state) {
      if (path.node.specifiers.length === 0 && isPolyfillSource(path.node.source.value)) {
        ...
        path.replaceWithMultiple(createImports([].concat(state.opts.polyfills, alwaysInclude), "import", state.opts.regenerator));
      }
    },
    Program: function Program(path, state) {
      ...
      path.get("body").forEach(function (bodyPath) {
        if (isRequire(bodyPath)) {
          ...
          bodyPath.replaceWithMultiple(createImports([].concat(state.opts.polyfills, alwaysInclude), "require", state.opts.regenerator));
        }
      });
    }
  };
...
var alwaysInclude = ["web.timers", "web.immediate", "web.dom.iterable"];